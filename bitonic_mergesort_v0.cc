...
int main(int argc, char* argv[]) {
    ...
    for (int step = 2; step <= size; step <<= 1) {
        for (int k = step >> 1; k > 0; k >>= 1) {
            if (rank & k) {
                MPI_Send(chunk, chunksize, MPI_DOUBLE, rank - k, 0,
                         MPI_COMM_WORLD);

                MPI_Recv(chunk, chunksize, MPI_DOUBLE, rank - k, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            } else {
                MPI_Recv(chunk + chunksize, chunksize, MPI_DOUBLE, rank + k, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                merge(chunk, chunksize << 1, rank & step);

                MPI_Send(chunk + chunksize, chunksize, MPI_DOUBLE, rank + k, 0,
                         MPI_COMM_WORLD);
            }
        }
